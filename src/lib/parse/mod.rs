...
pub fn parse_lines(lines: Vec<String>) -> ParseResult<Vec<ParsedLine>> {
        ...
        let command = match command_parse_result {
            ...
            None => {
                return Err(ParseError::SyntaxError(
                    i,
                    lines[i].to_owned(),
                    format!("Invalid or Unimplemented Operation: {}", parts[0]),
                ));
            }
        };
        ...
        if let LineCommandType::Memory = command.command_type() {
            ...
            let memory_segment = match memory_segment_parse_result {
                ...
                None => {
                    return Err(ParseError::SyntaxError(
                        i,
                        lines[i].to_owned(),
                        format!("Invalid Memory Segment: {}", parts[1]),
                    ));
                }
            };
            ...
            if let (MemorySegment::Constant, LineCommand::Pop) = (&memory_segment, &command) {
                return Err(ParseError::SyntaxError(
                    i,
                    lines[i].to_owned(),
                    format!("Invalid pop argument: constant"),
                ));
            }
            ...
            let memory_addr = match memory_addr_parse_result {
                ...
                Err(_) => {
                    return Err(ParseError::SyntaxError(
                        i,
                        lines[i].to_owned(),
                        format!("Invalid Memory Address: {}", parts[2]),
                    ));
                }
            };
